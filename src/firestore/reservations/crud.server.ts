...
import { makeConfirmationCode } from "../../utils/make-confirm-code"
...
export const reservationsDb = ({
	...
	const listByEvent = async ({ eventId }: { eventId: string }) => {
		const querySnapshot = await collectionRead.where("eventId", "==", eventId).get()
	...
	const checkReservation = async ({
		...
		const querySnapshot = await collectionRead.where("userId", "==", userId).where("eventId", "==", eventId).get()
		const docs = querySnapshot.docs.map((doc) => doc.data())
	...
	const checkUserReservations = async ({
		...
		const querySnapshot = await collectionRead.where("userId", "==", userId).where("eventId", "in", eventIdArray).get()
		const docs = querySnapshot.docs.map((doc) => doc.data())
